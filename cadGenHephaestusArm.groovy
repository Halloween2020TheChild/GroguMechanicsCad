...
return new ICadGenerator(){
				...
				if(linkIndex==1) {
					...
					brace = brace
							.union([
								...
								clearencelugMotorSide,
								clearencelugPassiveSide
							]
							).hull()
					def passiveSide = mountPassiveSideAlligned.union(passivLinkLug).hull()
					def motorSidePlate = CSG.hullAll([
						clearencelugMotorSide,
						mountMotorSide
					]);
					motorSidePlate=CSG.hullAll([
						motorSidePlate,
						motorSidePlate.toZMax().movez(centerlineToOuterSurfacePositiveZ-offsetOfLinks)
					])
					def motorSidePlatekw = CSG.hullAll([
						clearencelugMotorSidekw,
						mountMotorSidekw
					]);
					motorSidePlatekw=CSG.hullAll([
						motorSidePlatekw,
						motorSidePlatekw.toZMax().movez(centerlineToOuterSurfacePositiveZ-offsetOfLinks)
					])

					def center = CSG.unionAll([
						mountPassiveSideAlligned,
						mountMotorSide,
						clearencelugMotorSide,
						clearencelugPassiveSide
					])
					...
					.difference([
						springPathDriveMountKW,
						springPathPassiveMountKW
					])

					FullBracket.setManufacturing({incoming->return incoming.toZMin()})
					MotorMountBracket.setManufacturing({incoming->return incoming.rotx(180).toZMin().rotz(60)})
			...
			public ArrayList<CSG> generateBody(MobileBase b ) {
				...
				cameraHeight_parameterized = new LengthParameter("Camera Stand Height",cameraHeight_default,[
					cameraHeight_default+200,
					cameraHeight_default-200
				])
				...
				def mountLoacionsFeducials = [
					new TransformNR(0,-yOffsetFeducial,0,new RotationNR(180,0,0)),
					// feducial
					new TransformNR(0,yOffsetFeducial,0,new RotationNR(180,0,0)),
					// feducial
					new TransformNR(baseGrid+yOffsetFeducial,yOffsetFeducial,0,new RotationNR(180,0,0)),
					// feducial
					new TransformNR(yOffsetFeducial-baseGrid,0,0,new RotationNR(180,0,0)),
					// feducial
				...
				for(DHParameterKinematics dev:b.getAllDHChains()) {
					...
					def calibrationFramemountUnit =[]
					// Hardware to engineering units configuration
					LinkConfiguration conf = dev.getLinkConfiguration(0);
				 ...
				 */
					...
					if(devGet) {
						...
						poseInCal = dev.forwardOffset(dev.forwardKinematics(jointSpaceVect));
